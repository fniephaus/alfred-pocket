...
def main(_):
    ...
    if WF.update_available:
        WF.add_item(
            "An update is available!", autocomplete="workflow:update", valid=False
        )
    ...
    try:
        ...
        if not user_input[0] or (
            ...
            for category, action in zip(CATEGORIES, ACTIONS):
                WF.add_item(category, autocomplete="in:%s " % action, valid=False)
        else:
            links = get_links()
            if not links:
                WF.add_item(
                    "Your Pocket list is empty!", icon=get_icon("info"), valid=False
                )
                ...
                unread_items = [
                    l
                    for l in list(links.values())
                    if item_matches_category("mylist", l)
                    and link_matches_filter(search_query, l)
                ]
...
def link_matches_filter(user_input, link):
    ...
    return user_input.lower() in title.lower() or user_input.lower() in subtitle.lower()
...
def filter_and_add_items(links, user_input):
    ...
    for index, link in enumerate(links):
        if all(x in link for x in REQUIRED_KEYS):
            ...
            if (
                user_input.lower() in title.lower()
                or user_input.lower() in subtitle.lower()
            ):
                WF.add_item(
                    title,
                    subtitle,
    ...
    if WF._items == []:
        WF.add_item('No links found for "%s".' % user_input, valid=False)
...
def get_subtitle(item_count, time_updated, given_url, tags=None):
    time_updated = datetime.datetime.fromtimestamp(int(time_updated)).strftime(
        "%Y-%m-%d %H:%M"
    )
    ...
    if tags:
        tags = ["#%s" % x for x in list(tags.keys())]
...
def authorize():  # pragma: no cover
    ...
    if request_token:
        try:
            ...
            WF.save_password("pocket_access_token", user_credentials["access_token"])
