...
class Pocket(object):
    ...
    def _post_request(url, payload, headers):
        r = web.post(url, data=payload, headers=headers)
    ...
    def commit(self):
        ...
        """
        url = self.api_endpoints["send"]
        payload = {"actions": self._bulk_query}
    ...
    def get_auth_url(cls, code, redirect_uri="http://example.com"):
        auth_url = (
            ...
            f"?request_token={code}&redirect_uri={redirect_uri}"
        )
        return auth_url
    ...
    def auth(
        ...
        auth_url = (
            ...
            f"{code}&redirect_uri={redirect_uri}"
        )
        input(
            f"Please open {auth_url} in your browser to authorize the app and "
            "press enter:"
        )
