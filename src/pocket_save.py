...
def main(_):
    ...
    if args.tags:
        tags += [
            str(s.strip().strip("#").encode("utf-8")) for s in args.tags.split(",")
        ]
...
def parse_args(args):
    ...
    parser.add_argument(
        "--add-and-archive", dest="add_and_archive", action="store_true", default=None
    )
...
def get_link_from_clipboard():
    ...
    if stderr:
        return None
    parts = urllib.parse.urlsplit(clipboard)
